namespace Skyline.DataMiner.CICD.Tools.VisualStudioProjectVersionUpdater
{
    ...
    /// </summary>
    internal class ProjectFileProcessor
    {
        ...
        private void SetVersion(string version, int revision, XElement propertyGroupElement)
        {
            ...
            Console.WriteLine($"Updated File {FileSystem.Instance.Path.GetFileName(projectFile)} to version:{newVersion}");
        }
        ...
        private void SetNuGetVersion(string version, int revision, XElement propertyGroupElement)
        {
            ...
            {
                UpdateProjectProperty(ns + "PackageVersion", $"{newVersion}-{splitVersion[1]}", propertyGroupElement);
            }
            else
            {
                if (revision != 0)
                {
                    UpdateProjectProperty(ns + "PackageVersion", $"{newVersion}-prerelease", propertyGroupElement);
                }
                else
                {
